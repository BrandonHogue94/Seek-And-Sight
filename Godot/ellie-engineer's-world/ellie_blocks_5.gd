...
#func _input(event):
					...
					#is_dragging = true
					#if (get_parent().previous_dragged_brick == null || get_parent().last_dragged_brick == null):
						#get_parent().previous_dragged_brick = get_parent().last_dragged_brick
						#get_parent().last_dragged_brick = self
					#if (get_parent().previous_dragged_brick != null && get_parent().last_dragged_brick != null):
						#if self.global_position.distance_to(get_parent().previous_dragged_brick.global_position) < 90:
							#get_parent().previous_dragged_brick = get_parent().last_dragged_brick
							#get_parent().last_dragged_brick = self
